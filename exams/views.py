...
def home(request):
    ...
    try:
        ...
        latest_articles = BlogPost.objects.filter(is_published=True).select_related("category").order_by("-created_at")[:3]
    except Exception:
        latest_articles = []
    return render(request, "home.html", {"subjects": subjects, "latest_articles": latest_articles})
